...
class RandomMeshGenerator(object):
    def __init__(self, real_space, mesh_size, sample_size, attr_features=1,grid_input=np.array([]),grid_boundary=np.array([])):
    ...
    def sample_G(self, is_random=True,inbd=None):
        if not is_random:
            self.idx = torch.arange(self.n)
        else:
            n=len(inbd)
            inbd=torch.tensor(inbd)
            perm = torch.randperm(n)[:self.m]
            self.idx = inbd[perm]
    ...
    def sample_bd(self,otbd_idx):
        self.outdomain_idx=otbd_idx
        perm = torch.randperm(self.n)
        perm = perm.cpu().numpy().tolist()
        for i in range(len(self.outdomain_idx)):
            perm.remove(self.outdomain_idx[i])
...
#0812修改
    def ball_connectivity_dist(self, r, is_forward=False):
        self.pwd = sklearn.metrics.pairwise_distances(self.grid_sample)
        
        self.edge_index = np.vstack(np.where(self.pwd <= r))
        self.n_edges = self.edge_index.shape[1]
    ...
    def ball_connectivity_dist_0(self, r, is_forward=False,ns=10,tri_edge=None):
        self.pwd = sklearn.metrics.pairwise_distances(self.grid_sample)
        tri_edge = tri_edge.T
        ...
        self.edge_index = RandomMeshGenerator.deduplicate_rows(self.edge_index.T).T
        self.n_edges = self.edge_index.shape[1]
        if is_forward:
            print(self.edge_index.shape)
            self.edge_index = self.edge_index[:, self.edge_index[0] >= self.edge_index[1]]
    ...
    def ball_connectivity_dist_1(self, r, is_forward=False,ns=10,tri_edge=None,cor=None):
        self.pwd = sklearn.metrics.pairwise_distances(self.grid_sample)
        cor=cor.astype(int)
        tri_edge = tri_edge.T
    ...
    def ball_connectivity_dist_2(self, r, is_forward=False,ns=10,tri_edge=None):
        self.pwd = sklearn.metrics.pairwise_distances(self.grid_sample)
        tri_edge = tri_edge.T
    ...
    def ball_connectivity_bdcr(self, radius0,coe, is_forward=False):
        pwd = sklearn.metrics.pairwise_distances(self.grid_sample)
        self.coe=coe
        edge_index_id1=np.array([],dtype=int)
        edge_index_id2=np.array([],dtype=int)
        for p in range(self.coe.shape[0]):
            temp=[]
            radius_cr=radius0*coe[p][0]*coe[p][1]
            temp=np.where(self.pwd[p]<=radius_cr)[0]
            for pp in range(temp.shape[0]):
                edge_index_id1=np.append(edge_index_id1,int(p))
                edge_index_id2=np.append(edge_index_id2,int(temp[pp]))
        self.edge_index = np.vstack([edge_index_id1,edge_index_id2])
        self.n_edges = self.edge_index.shape[1]
    ...
    def attributes_dist_IC(self, theta=None):
        # pwd = sklearn.metrics.pairwise_distances(self.grid_sample)
        theta = theta[self.idx] 
        edge_attr = np.zeros((self.n_edges,1))
        self.edge_index=torch.tensor(self.edge_index).to(torch.int32)
        

        for p in range(self.n_edges):
            edge_attr[p]=self.pwd[self.edge_index[0][p]][self.edge_index[1][p]]

        return torch.tensor(edge_attr, dtype=torch.float) 

    

    def attributes_dist_neo7(self, theta=None):
        # pwd = sklearn.metrics.pairwise_distances(self.grid_sample)
        theta = theta[self.idx] 
...
class RandomMultiMeshGenerator(object):
    ...
    def sample_0927(self):
        ...
        index = 0
        for l in range(self.level):
            idx_l = perm[index: index+self.m[l]]
            grid_sample_l = self.grid[idx_l]  # 创建grid_sample_l的副本
            self.idx.append(idx_l)
            self.grid_sample.append(grid_sample_l)
            index = index + self.m[l]
        self.idx_all = perm[:index]
        self.grid_sample_all = self.grid[self.idx_all]
